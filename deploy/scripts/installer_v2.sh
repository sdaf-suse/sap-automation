...
function sdaf_installer() {
	...
	if [ ! -f .terraform/terraform.tfstate ]; then
		...
		if ! $IAC_TOOL -chdir="${terraform_module_directory}" init -upgrade=true -input=false \
		...
		if local_backend=$(grep "\"type\": \"local\"" .terraform/terraform.tfstate); then
			if [ -n "$local_backend" ]; then
				...
				if $IAC_TOOL -chdir="${terraform_module_directory}" init -migrate-state --backend-config "path=${param_dirname}/terraform.tfstate"; then
					return_value=$?
			...
			if $IAC_TOOL -chdir="${terraform_module_directory}" init -force-copy \
				--backend-config "subscription_id=${terraform_storage_account_subscription_id}" \
				--backend-config "resource_group_name=${terraform_storage_account_resource_group_name}" \
				--backend-config "storage_account_name=${terraform_storage_account_name}" \
				--backend-config "container_name=tfstate" \
	...
	if [ ${DEBUG:-False} == True ]; then
		...
		$IAC_TOOL -chdir="${terraform_module_directory}" output -json
	fi
	...
	if [ "${deployment_system}" == sap_deployer ]; then
		...
		DEPLOYER_KEYVAULT=$($IAC_TOOL -chdir="${terraform_module_directory}" output -no-color deployer_kv_user_name | tr -d \")

		app_config_id=$($IAC_TOOL -chdir="${terraform_module_directory}" output -no-color deployer_app_config_id | tr -d \")

		app_service_name=$($IAC_TOOL -chdir="${terraform_module_directory}" output -no-color webapp_url_base | tr -d \")

		app_service_deployment=$($IAC_TOOL -chdir="${terraform_module_directory}" output -no-color app_service_deployment | tr -d \")
