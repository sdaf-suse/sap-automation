...
function sdaf_remover() {
	...
	if [ ! -f .terraform/terraform.tfstate ]; then
		...
		if ! $IAC_TOOL -chdir="${terraform_module_directory}" init -force-copy -upgrade=true \
			--backend-config "subscription_id=${terraform_storage_account_subscription_id}" \
			--backend-config "resource_group_name=${terraform_storage_account_resource_group_name}" \
			--backend-config "storage_account_name=${terraform_storage_account_name}" \
			--backend-config "container_name=tfstate" \
	...
	if [ "$deployment_system" == "sap_deployer" ]; then
		$IAC_TOOL -chdir="${terraform_module_directory}" destroy -var-file="${var_file}"
		...
		$IAC_TOOL -chdir="${terraform_bootstrap_directory}" destroy -var-file="${var_file}" "${approve}" -var use_deployer=false
