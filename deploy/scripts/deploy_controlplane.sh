...
if [ 2 -eq $step ]; then
	...
	if ! $IAC_TOOL -chdir="${terraform_module_directory}" output | grep "No outputs"; then
		...
		if [ -z "$REMOTE_STATE_SA" ]; then
			REMOTE_STATE_RG=$($IAC_TOOL -chdir="${terraform_module_directory}" output -no-color -raw sapbits_sa_resource_group_name | tr -d \")
		fi
		if [ -z "$REMOTE_STATE_SA" ]; then
		...
		if [ -z "$STATE_SUBSCRIPTION" ]; then
			STATE_SUBSCRIPTION=$($IAC_TOOL -chdir="${terraform_module_directory}" output -no-color -raw created_resource_group_subscription_id | tr -d \")
		fi
